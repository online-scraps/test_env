    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            @if (isset($sales->status_id))
                ...
                @if ($sales->status_id == 3)
                    $("#salesForm :input").prop("disabled", true);
                    Swal.fire("Sales Order canceled!");
                @endif
            @endif
            ...
            function getStockItemDetails(itemId, cntr) {
                let url = '{{ route('custom.get-total-sales-order', ':id') }}';
                ...
                $.get(url).then(function(response) {
                    ...
                    $('#salesUnitPrice-' + cntr).val(response.item_price);
                ...
                $.each(currentBarcode, function(code) {
                    ...
                    if (ocurrance_count > 3) {
                        ...
                        $.each(check_exist_or_not, function(code) {
                            // console.log(barcodeList[check_exist_or_not[code]]['item_id'] != item_id)
