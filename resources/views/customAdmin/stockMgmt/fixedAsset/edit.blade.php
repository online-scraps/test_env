...
@section('content')
    ...
    <form id="fixedAssetEntryForm" action="{{url($crud->route).'/'.$stock->id}}" method="PUT">
        @method('PUT')
        @csrf
            ...
            <div class="row mt-3">
                <div class="col-xl-4 col-lg-4 col-sm-6">
                    <div class="input-group mb-3">
                        <select class="form-select store_id" id="store_id"  name="store_id">
                            <option value="" disabled selected>Select a store</option>
                        @foreach($storeList as $key => $list)
                                <option {{$stock->store_id == $key ?'selected':''}} value="{{$key}}">{{$list}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="input-group mb-3">
                        ...
                        <select class="form-select" id="adjustment_no" name="adjustment_no">
                            @foreach ($adjustmentNumbers as $code => $codeId)
                                <option value="{{ $codeId }}" {{ ($codeId == $stock->adjustment_no) ? 'selected' : ''  }}>{{ $code }}</option>
                            @endforeach
                        </select>
                        <span class="input-group-text bg-primary text-white"  onclick="loadModal(this, '6')">
                            +
                        </span>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-sm-6">
                    <div class="input-group mb-3">
                        ...
                        <input type="checkbox"  name="itemWiseDiscount" id="depreciationCheckbox" {{!$stock->flat_depreciation?"checked" :'' }} class="mt-2 mx-2">
                    </div>
                </div>
            </div>
        </div>
            ...
            <tr>
                ...
                <td>
                    <div class="input-group">
                        ...
                        <input type="hidden" name="itemStockHidden[{{$key}}]" value="{{$item->mst_item_id}}"  class="itemStockHidden">
                    </div>
                </td>
                <td>
                    <div class="input-group">
                        <input id="availableQty-{{$key}}"  data-cntr="{{$key}}" type="number" min="0" class="form-control p-1 availableQty" name="available_total_qty[{{$key}}]" value="{{$itemQty}}"  size="1" readonly>
                    </div>
                </td>
                <td>
                    <div class="input-group">
                        ...
                        <input type="number" min="0" class="form-control p-1 addQty" value="{{$item->add_qty}}"  style="border-top-left-radius: 0;border-bottom-left-radius: 0;" {{$item->barcodeDetails->count() ? "readonly='true'" :'' }} name="add_qty[{{$key}}]" placeholder="Qty" id='addQty-{{$key}}'data-cntr="{{$key}}" size="1" {{$multiple_barcode?'readonly':''}}>

                        {{-- <input type="number" min="0" class="form-control p-1 addQty" value="{{$item->add_qty}}"  style="border-top-left-radius: 0;border-bottom-left-radius: 0;"  name="add_qty[{{$key}}]" placeholder="Qty" id='addQty-{{$key}}' data-cntr="{{$key}}" size="1" {{$multiple_barcode?'readonly':''}}> --}}
                    </div>
                </td>
                <td>
                    <div class="input-group">
                        <input type="number" min="0" class="form-control p-1 freeQty" name="free_item[{{$key}}]" value="{{$item->free_item}}" placeholder="Free item" id="freeQty-{{$key}}" data-cntr="{{$key}}" size="1">
                    </div>
                </td>
                <td >
                    <div class="input-group">
                ...
                <td>
                    <div class="input-group">
                        <input id="itemTax-{{$key}}" data-cntr="{{$key}}"  type="number" min="0" class="form-control p-1 itemTax" name="tax_vat[{{$key}}]" value="{{$item->tax_vat}}"  size="1" readonly>
                                ...
                                <td>
                    <div class="input-group">
                        <input type="number" min="0" max="100" class="form-control p-1 fireRepeater depreciation" name="depreciation[{{$key}}]" value="{{$item->depreciation}}"  {{$stock->flat_depreciation?"disabled='true'" :'' }} placeholder="Depreciation %" id="depreciation-{{$key}}" size="1" data-cntr="{{$key}}">
                    </div>
                </td>
                <td>
                    <div class="input-group">
            ...
            <tr id="repeater" class="d-none">
                ...
                <td>
                    <div class="input-group">
                        ...
                        @endif
                        <input type="number" min="0" class="form-control p-1 addQty" placeholder="Qty" size="1" style="border-top-left-radius: 0;border-bottom-left-radius: 0;" {{$multiple_barcode?'readonly':''}}>
                    </div>
                </td>
                <td>
                    <div class="input-group">
                        <input type="number" min="0" class="form-control p-1 freeQty" placeholder="Free item" size="1">
                    </div>
                </td>
                <td>
                    <div class="input-group">
                ...
                <td>
                    <div class="input-group">
                        <input type="number" min="0" max="100" class="form-control p-1 fireRepeater depreciation" {{$stock->flat_depreciation?"distabled='true'" :'' }} placeholder="%" size="1">
                    </div>
                </td>
                <td>
                    <div class="input-group">
            ...
            <div class="row">
                <div class="col-md-6 col-sm-12">
                    <div class="input-group mb-3">
                        ...
                        <input type="number" min="0" max="100" id="flatDiscount" name="flat_depreciation" {{!$stock->flat_depreciation ?" disabled='true'":'' }} Value="{{$stock->flat_depreciation}}" class="form-control" placeholder="Depreciation">
                    </div>
                    <div class="input-group mb-3">
                    ...
                    <div class="custom-file">
                        <input type="file" class="custom-file-input" id="inputGroupFile01" name="upload_bill" onchange="document.getElementById('uploadedBill').src = window.URL.createObjectURL(this.files[0])">
                        <label class="custom-file-label" for="body_imgUserImage">Choose file</label>
                    </div>
                </div>
                    @if (isset($stock->upload_bill))
                        <img src="{{ '/storage/uploads/'.$stock->upload_bill }}" class="img-thumbnail" alt="..." id="uploadedBill" >
                    @endif
                    <div class="input-group mb-3">
                        <span class="input-group-text" >Remarks</span>
                        <textarea class="form-control comment" name="comments"  col="5" placeholder="Remarks" > {{ isset($stock->comments) ? $stock->comments : old('comments')  }} </textarea>
                    </div>
                </div>
                <div class="col-md-6 col-sm-12">
                    <table class="table table-sm table-bordered">
                        ...
                        <tr>
                            ...
                            <td>
                                <input id="st_depreciation_amount" type="number" min="0" name="total_discount" value="{{$stock->total_depreciation}}" class="form-control" readonly>
                            </td>
                        </tr>
                        <tr>
        ...
        <div class="main-container mb-4">
            <div class="row ">
                <div class="col d-flex justify-content-end">
                    @if($stock->status_id !== \App\Models\SupStatus::APPROVED)
                        ...
                        @endif
                    @endif
                    <a href="{{url($crud->route)}}"><i  class="btn btn-danger me-1">{{$stock->status_id == \App\Models\SupStatus::APPROVED ?'Back':'Cancel' }}</i></a>
                </div>
            </div>
        </div>
    </form>
