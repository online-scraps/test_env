        ...
        $(document).ready(function() {
            ...
            @if (isset($sales->status_id))
                @if ($sales->status_id == 2)
                    $("#stockTransferForm :input").prop("disabled", true);
            ...
            function setIdToRepeater(cntr, cloneTr) {
                let classArr = ['salesItemStock', 'salesAvailableQty',
                    'salesAddQty', 'salesUnit', 'salesUnitPrice', 'salesDiscount', 'salesTax', 'salesAmount',
                    'itemDestroyer'
                ];
                let nameArr = ['item_id', 'sales_availableQty', 'total_qty', 'unit_id',
            ...
            $(".salesBatchNo").change(function() {
                ...
                if (batchId) {
                    ...
                    $.get(url).then(function(response) {
                        $('#salesBatchQty-' + cntr).val(response.batchDetail.batch_qty);
                        $('#salesUnitPrice-' + cntr).val(response.batchDetail.batch_price);
                    })
            ...
            $('#cancel').on('click', function() {
                $('#status').val({{ \App\Models\SupStatus::CANCELLED }});
                ...
                submitHandler: function(form) {
                    Swal.fire({
                        ...
                        if (confirmResponse.isConfirmed) {
                            ...
                            $("#stockTransferForm :input").prop("disabled", false);
                            let data = $('#stockTransferForm').serialize();
                            let url = form.action;
                            axios.post(url, data)
                                .then((response) => {
            ...
            $('#barcodeSave').on('click', function() {
                ...
                let url = '{{ route('custom.stock-transfer-barcode', [':itemId', ':batchId']) }}';
                ...
                axios.get(url)
                    .then((response) => {
                        if (response.data.status === 'success') {
                            ...
                            {
                                $select.append('<option value=' + value['id'] + '>' + value['name_en'] + '</option>'); // return empty
                            });
                        } else {
                            Swal.fire({
                                title: 'Are you sure?',
