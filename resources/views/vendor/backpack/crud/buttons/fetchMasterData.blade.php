@php
    $fields = ['id', 'sup_org_id', 'store_id', 'is_active', 'is_super_data', 'created_by', 'updated_by', 'deleted_by', 'is_deleted', 'deleted_at', 'deleted_uq_code', 'created_at', 'updated_at', 'sup_data_id', 'is_consumed'];

    $thead = collect($crud->model->first())
        ->keys()
        ->toArray();
    $tbody = $crud->model
        ->where('is_super_data', true)
